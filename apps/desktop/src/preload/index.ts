...
const api: DesktopApi & { onAppCommand(listener: (command: string) => void): () => void } = {
  ...
  themes: {
    ...
    bootstrap: () => ipcRenderer.invoke("themes:bootstrap"),
    setPreference: (preference) => ipcRenderer.invoke("themes:set-preference", preference),
    onResolvedModeChanged: (listener) => subscribe("themes:resolved-mode", listener),
  },
  settings: {
    ...
    getAgentDefaults: () => ipcRenderer.invoke("settings:get-agent-defaults"),
    setAgentDefaults: (defaults) => ipcRenderer.invoke("settings:set-agent-defaults", defaults),
  },
  workspace: {
  ...
  weixin: {
    ...
    onEvent: (listener) => subscribe("weixin:event", listener),
  },
